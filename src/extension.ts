...
export function activate(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand(
    "react-native-move-unistyles.helloWorld",
    async () => {
      const { activeTextEditor, showInputBox, showInformationMessage } =
        vscode.window;
      ...
      if (!userStylesName) {
        showInformationMessage(
          "react-native-move-unistyles expects the style name"
        );
        return;
      }
      ...
      const configuration = vscode.workspace.getConfiguration(
        "react-native-move-unistyles"
      );
      const userInsertOrder =
        configuration.get("insertOrder") || insertOrder.start;
      ...
      if (!stylesText) {
        showInformationMessage(
          "react-native-move-unistyles styles to be selected"
        );
        return;
      }
      ...
      activeTextEditor.edit((edit) => {
        ...
        if (!editorCode.includes("StyleSheet.create")) {
          if (
            ...
            const importStatements = editorCode.match(/import .+ from .+;/g);
            if (importStatements) {
              const lastImportStatement = importStatements.pop();
              if (lastImportStatement) {
                const lastImportIndex =
                  document.positionAt(
                    editorCode.lastIndexOf(lastImportStatement) +
                      lastImportStatement.length
                  ).line + 1;
                edit.insert(
                  new vscode.Position(lastImportIndex, 0),
                  'import { StyleSheet } from "react-native-unistyles";\n'
                );
              }
            } else {
          ...
          const newStyleSheet = withStyleSheetCreate(
            userStylesName,
        ...
        const startOfStyleSheetIdx = document.positionAt(
          editorCode.indexOf("StyleSheet.create")
        ).line;

        /** add to the start of the StyleSheet */
        if (userInsertOrder === insertOrder.start) {
          const stylesToBeAppended = `  ${userStylesName}: ${formatStyles(
            stylesText
          )},\n`;
          edit.insert(
            new vscode.Position(startOfStyleSheetIdx + 1, 0),
            stylesToBeAppended
          );
          return;
        }
        ...
        const endOfStyleSheetIdx = document.positionAt(
          editorCode.indexOf("})", startOfStyleSheetIdx)
        ).line;
         ...
         */
        const lineToDelete = activeTextEditor.document.lineAt(
          endOfStyleSheetIdx - 1
        );
        edit.delete(lineToDelete.rangeIncludingLineBreak);

        const stylesToBeAppended = `  },\n  ${userStylesName}: ${formatStyles(
          stylesText
        )},\n`;
        edit.insert(
          new vscode.Position(endOfStyleSheetIdx - 1, 0),
          stylesToBeAppended
        );
      });
    }
